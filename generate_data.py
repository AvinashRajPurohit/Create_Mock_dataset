...
def write_row(order_id, product, order_date, address):
    ...
    quantity_orderd = np.random.geometric(p=1.0-(1.0/price), size=1)[0]
    return [order_id, product, quantity_orderd, price, date, address]
...
for month_value in range(1,13):
  ...
  while order_amount > 0:
      ...
      date = genrate_random_time(month_value)
      ...
      if product == 'iphone':
        ...
        if random.random() < 0.7:
          df.loc[i] = write_row(order_id, "LG Washing Machine", date, address)
          i+=1
